...
public:
  ...
  explicit ThreadPool(std::size_t numThreads = std::max(std::thread::hardware_concurrency(),(uint32_t)1))
      ...
      mWorkers.emplace_back([this] {
        while (true) {
          Task task;
          {
            ...
            // 释放锁，并将调用线程置于休眠状态，直到满足以下条件之一
            // (mStop变为true或者任务容器mTasks不为空)
    ...
    mManager = std::thread([this] {
      while (!mStop) {
        ...
        // the number of workers and if the task-to-worker ratio is greater than
        // 2
        ...
        // and if the number of workers is greater than the initial number of
        // threads
  ...
  auto addTask(F &&f, Args &&...args)
    ...
    // typedef for the return type of the function or function object F when it
    // is called with the arguments Args....
    using return_type = typename std::result_of<F(Args...)>::type;
    // creates a std::shared_ptr to a std::packaged_task that wraps a callable
    // object f and its arguments args.... wraps a callable object (such as a
    // function, function pointer, or function object) and allows you to invoke
    // it later and obtain its result through a std::future. The template
    // parameter return_type() represents the return type of the callable
    // object.
